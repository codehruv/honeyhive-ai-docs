...
Using this method, we run the OpenAI call on your side and ingest the response later without acting as a proxy layer.
...
Alternatively, you can leverage our prompt CI/CD capabilities and just call the current deployed prompt within a project without specifying all the parameters. Using this method, we automatically route your requests to the model-prompt configuration that you choose to deploy within the platform.
