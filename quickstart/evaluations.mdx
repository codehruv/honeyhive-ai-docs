...
`log_run` accepts 3 necessary parameters:
...
`log_run` also accepts 1 optional parameter:
1. `metrics`: an optional field specifying any recorded metrics of interest for the run (ex. cost, latency, etc)
